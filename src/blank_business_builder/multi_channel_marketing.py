...
class MultiChannelCampaignOrchestrator:
    ...
    def allocate_budget(
        ...
        if total_roi_score == 0:
            # Fallback to equal split if all ROI scores are somehow 0
            per_channel = campaign.budget / len(campaign.channels)
            return {channel: per_channel for channel in campaign.channels}

        # Allocate budget proportionally to ROI scores
        # BOLT OPTIMIZATION: Hoisted division out of loop and used dictionary comprehension
        allocation_factor = campaign.budget / total_roi_score
        return {channel: allocation_factor * roi_scores[channel] for channel in campaign.channels}
