...
**Action:** When gathering status counts across an entire associated table, use a much more efficient `GROUP BY` query (`group_by(AgentTask.status)`) combined with a simple Python iteration mapping the output. This greatly mitigates event loop blocking risks from synchronous IO delays under load.## 2025-05-08 - Fast Budget Allocation and ZeroDivision Prevention
**Learning:** O(N) repetitive division calculations inside loops, combined with multiple variable assignment dictionaries can be fully hoisted and wrapped in native C-level dictionary comprehensions. Python performs dictionary creation much faster via comprehensions than loop-based instantiation. Also, mathematical operations using dynamically changing or zero-based aggregations (e.g. `total_roi_score`) must be guarded tightly.
**Action:** Always extract division calculations derived from aggregated totals outside of loops if the denominator is constant, turning it into an `allocation_factor`. Utilize dictionary comprehensions for single-pass mappings. Always include `if not items` and `if total == 0` guard clauses before mathematical logic on collections.
