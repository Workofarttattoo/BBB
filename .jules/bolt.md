...
## 2026-03-05 - Optimize Budget Allocation
**Learning:** Found repetitive dict assignments (`allocation[channel] = campaign.budget * (roi_scores[channel] / total_roi_score)`) inside loops where parts of the calculation could be hoisted, avoiding redundant division operations. Also realized `total_roi_score` might end up as zero leading to potential `ZeroDivisionError`.
**Action:** When working on math-intensive loops like performance allocation, extract constant arithmetic logic out of loops. E.g. setting `allocation_factor = budget / total` and using dictionary comprehensions drastically reduces repeated execution context switches. Additionally, ensure there are safeguards against ZeroDivisionErrors, even for fallback scores.
