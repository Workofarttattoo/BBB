...
## 2025-04-24 - Optimize module-level constant keyword mappings
**Learning:** Found O(N*M) mapping logic in list comprehensions and iterative generators where inline keyword sets were used (e.g. `any(kw in ... for kw in ['ai', 'ml', ...])`). Creating sets or lists over and over inside the iteration represents a significant performance bottleneck in deeply nested tight loops in code like `quantum_stack_optimizer.py` and `expert_integration.py`. Extracting these mappings to module-level constants reduces memory allocation and computational overhead significantly without sacrificing readability.
**Action:** Extract inline lists to module-level constants for functions like `_map_task_to_domain` and `_calculate_quantum_resource_allocation`.
