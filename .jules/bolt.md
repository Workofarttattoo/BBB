...
## 2024-05-24 - Optimization: O(1) tracking for IN_PROGRESS tasks
**Learning:** `AutonomousBusinessOrchestrator` heavily relies on queue iterations `[t for t in self.task_queue if t.status == TaskStatus.IN_PROGRESS]` on loops that run every few seconds, which is O(N). Because task queues can get extremely large in simulation/run-time (N > 10,000 tasks), these constant iterations cause CPU bottlenecking and major performance degradations. Adding O(1) state variables such as `self.in_progress_tasks` for constant time lookups scales much better.
**Action:** When tracking dynamic states that are repeatedly queried (e.g. `IN_PROGRESS`, `BLOCKED`), use Sets or counters (like `task_status_counts`) initialized upfront instead of List comprehensions across a full collection. Ensure custom hashable dataclass rules are properly aligned.
