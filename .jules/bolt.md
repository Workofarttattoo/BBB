...
## 2026-03-04 - Optimize Task Queue Iteration for Active Tasks
**Learning:** In `AutonomousBusinessOrchestrator`, methods like `_reconcile_orphaned_in_progress_tasks` and `_execute_tasks_parallel` iterated over the entire historical `task_queue` to find a few `IN_PROGRESS` tasks. Since `task_queue` grows unboundedly as the business runs, this O(N) list comprehension inside the main run loop (which runs frequently) becomes a serious bottleneck over time.
**Action:** When managing an ever-growing list of events or tasks, always use a separate O(1) tracking collection (e.g., `in_progress_tasks` Set) combined with a lookup dictionary (e.g., `task_map`) to track the small subset of currently active items, avoiding O(N) scans over historical data.
